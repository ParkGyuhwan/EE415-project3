...
process_execute (const char *file_name) 
{
  ...
  char *nxt_ptr;
  char *name_prs;
  char fname_copy[strlen(file_name)+1];
  tid_t tid;
  ...
  /* parse the string file_name @@@@@@@@@@@@@@@@ modified */
  strlcpy (fname_copy, file_name, strlen(file_name)+1);
  name_prs = strtok_r(fname_copy, " ", &nxt_ptr);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (fname_copy, PRI_DEFAULT, start_process, fn_copy); //modified
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
  ...
  /* make tokens and calculate number of tokens */
  char* token;
  char* save_ptr;
  int num_token = 0;
  char **tokens_list;
  ...
  token = strtok_r(copy_file_name, " ", &save_ptr);
  while(token != NULL){
    num_token++;
    token = strtok_r(NULL, " ", &save_ptr);
  }
  tokens_list = (char **)malloc(sizeof(char *) * num_token);
  ...
  if(success) {
    argument_stack(tokens_list, num_token, &if_.esp);
  }
...
/* stack : Argv|word-align|argv address|main|fake address(0) */
...
struct thread *get_child_process(int pid)
{
	...
	for(e=list_begin(child_list); e != list_end(child_list);
      e=list_next(e))
	{
		child_process = list_entry(e,struct thread,childelem);
		if(pid == child_process->tid)
...
process_add_file(struct file *f)
{
	...
	int nxt_fd = thread_current()->next_fd;
...
struct file *process_get_file(int fd)
{
	...
	{
		goal_f = thread_current()->file_descriptor[fd];
		return goal_f;
	}
	else
	{
...
bool handle_mm_fault (struct vm_entry *vme)
{
  ...
  uint8_t *paddr = palloc_get_page(PAL_USER);
  ...
  if(install_page(vme->vaddr, paddr, vme->writable) == false)
	{
